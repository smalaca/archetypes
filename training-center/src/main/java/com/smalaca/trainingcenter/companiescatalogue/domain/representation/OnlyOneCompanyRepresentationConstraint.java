...
class OnlyOneCompanyRepresentationConstraint implements RepresentationConstraint {
    private final RepresentationRepository repository;

    OnlyOneCompanyRepresentationConstraint(RepresentationRepository repository) {
        this.repository = repository;
    }

    @Override
    public boolean canRepresent(RepresentativeId representativeId, CompanyId companyId, BusinessUnitId businessUnitId) {
        return !repository.isRepresentingAnyOtherCompany(representativeId, companyId);
    }
}
