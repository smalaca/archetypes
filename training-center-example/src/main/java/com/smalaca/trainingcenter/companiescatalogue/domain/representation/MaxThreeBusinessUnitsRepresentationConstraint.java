...
class MaxThreeBusinessUnitsRepresentationConstraint implements RepresentationConstraint {
    private static final int MAX_BUSINESS_UNITS = 3;
    private final RepresentationRepository repository;

    MaxThreeBusinessUnitsRepresentationConstraint(RepresentationRepository repository) {
        this.repository = repository;
    }

    @Override
    public boolean canRepresent(RepresentativeId representativeId, CompanyId companyId, BusinessUnitId businessUnitId) {
        return repository.countBusinessUnitsRepresentedIn(representativeId, companyId) < MAX_BUSINESS_UNITS;
    }
}
